(function($) { // Hide scope, no $ conflict
	...
	SUPER.init_before_scrolling_to_message_popup = function($proceed, $form, $scroll){
        if($form.parents('.super-popup-content:eq(0)').length){
            $form.parents('.super-popup-content:eq(0)').animate({
            	scrollTop: 0
            }, 1000);
	...
	SUPER.init_show_preview_popup = function($form_id){
	...
	SUPER.init_responsive_popup = function($classes, $new_class, $window_classes, $new_window_class) {
	...
	SUPER.init_popups.close = function($force_close) {
		...
		if($wrapper.length){
			...
			if( ($settings.disable_closing!='true') || ($force_close==true) ) {
				$wrapper.animate({
					opacity: 0
				}, parseInt($settings.fade_out_duration/2), function() {
					$wrapper.css('z-index','');
	...
	$(document).ready(function(){
		...
		$(window).click(function(e){
			...
			if(!$($target).parents().andSelf().is($popup)) {
